...
\bigbreak
Taken together, existing work has applied win-probability approaches for outcome prediction and action evaluation in esports, but leaves open the question of how to evaluate mid-round, sequential decisions. In particular, there is limited prior work that assesses saving decisions in Counter-Strike through a win-probability framework that models this type of mid-round decision. The present work seeks to address this gap by framing saving as a sequential decision problem and evaluating its optimality through a win-probability lens that accounts for mid-round state and long-term winning probability. In doing so, we are able to bridge predictive win-probability modeling with decision-theoretic evaluation, building a principled framework for assessing when sacrificing the current round may be optimal in maximizing the probability of winning the match.
